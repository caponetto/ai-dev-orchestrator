...
- **Speculative resource exhaustion** — Claims like "clients will accumulate unboundedly" or "goroutines will grow without limit" require evidence that cleanup/deadline paths are absent AND that ingress/TCP/proxy timeouts cannot bound connection lifetime. Without both, cap at `minor` and move to risks as an operational design question.
- **Policy tradeoffs framed as defects** — Long-lived connections authorized at establishment (SSE, WebSocket, gRPC stream, Kubernetes watch) are often intentional. Flag as `major` only if you can show a concrete harm path (data leak after revocation, cross-tenant bleed). Otherwise frame as `minor` hardening or a risks entry: "Consider periodic re-auth / max connection lifetime — policy decision."
- **False equivalence to external systems** — Do not claim a design "unlike Kubernetes watches" unless you cite how the referenced system's auth/lifetime semantics actually differ in a security-relevant way.
