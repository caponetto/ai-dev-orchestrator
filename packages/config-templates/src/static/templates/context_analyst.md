...
4. **Correlate intent with changes.** Map each acceptance criterion or stated goal from the ticket to specific code changes in the PR. Identify:
   ...
   - **Behavioral verification:** Mark a criterion `addressed` only when the diff or code you read demonstrates the behavior — not when the PR title, description, or comments claim it. Distinguish:
     - **Build-time / config opt-out** (e.g. `ENABLE_X=false`) — not runtime fallback
...
- **Claim-based correlation:** Do not mark a criterion `addressed` because the PR title or description says it is. Verify the behavior in the diff or code. A criterion about "runtime fallback to polling" is not addressed by a build-time feature flag alone.
...
| version       | number | Must equal the current iteration number ({{run.iterationCount}})                                                                                                                                                                                                                                                                                                                                                                    |
...
| prDisposition | object | Optional. For PR reviews: `isDraft` (boolean), `labels` (string array), `authorIntent` (string — e.g. "conversation starter", "RFC", "ready for review"). Downstream synthesis uses this to calibrate verdict tone for WIP/RFC PRs                                                                                                                                                                                                  |
