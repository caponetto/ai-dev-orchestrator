...
6. **Validate reasoning chain — MANDATORY GATE.** For each remaining critical or major finding, apply the checks below. If a finding fails ANY check, you MUST downgrade it to `minor`. Do not rationalize exceptions. Do not keep it at `major` because the code pattern "looks like" it could be a problem. Do not keep it at `major` because multiple reviewers agree on it. The standard is: **does the cited evidence prove the claimed consequence, or does it merely prove a code structure from which the consequence is inferred?**
   ...
   **Gate A — Cross-layer evidence:**
   ...
   - If the evidence is entirely from one layer (e.g., client/frontend) but the claimed consequence depends on how another layer (e.g., server/backend/API/controller) behaves, downgrade to `minor` unless the reviewer quoted code from that other layer proving the behavior. This is the single most common false-positive pattern. Specifically:
     - A fallback pattern (`filteredData ?? allData`) does NOT prove two data sources return different results.
     - Cleanup/revalidation logic (e.g., an effect that "clears incompatible X after reload") does NOT prove X can actually become incompatible — that depends on whether the data source returns different values under different conditions.
     - An async reload or deferred-write pattern does NOT prove a race condition unless the reviewer proved that the data can actually change between the two operations.
     - **Litmus test:** "If the other layer returns the same data regardless of context, does this finding still hold?" If the answer is "no" or "unclear" → downgrade to `minor`.
   ...
   **Gate D — Logical soundness:**
   ...
   - **Variable-name inference is not evidence.** Variable names suggesting filtering, scoping, or context-dependence (e.g., `filteredValues`, `scopedOptions`, `userSpecificData`) do not prove the underlying data source implements that behavior. A variable named "filtered" may receive the same data as "all" if the source does not actually filter.
   - **Repetition from prior runs is not evidence.** If a finding was flagged in a previous review iteration and NOT substantiated with new evidence in this iteration, treat it on its current merits alone. Do not escalate because it "keeps coming up."
...
7. **Promote unaddressed acceptance criteria (if correlation data is present).** If the canonical specification includes a `correlation` object with `notAddressed` criteria, evaluate whether each represents a completeness gap that would prevent the feature from working as described. If a `notAddressed` criterion means the change is functionally incomplete (e.g., a file described in the PR body is absent from the diff, a required integration is missing, a prerequisite is unmet), create a finding with category `correctness`, severity `major`, source `["canonical_specification"]`, and evidence quoting the spec's note explaining the gap. This is not inventing a finding — it is elevating a verified factual gap that the spec analyst already documented with evidence. Only do this for completeness gaps that affect whether the feature works, not for criteria that are merely "not yet tested" or "partially addressed." If no `correlation` object is present in the specification, skip this step.
8. **Reconcile acceptance criteria against findings (if correlation data is present).** If the canonical specification includes `addressed` and `partiallyAddressed` criteria in its `correlation` object, cross-check them against the synthesized findings. If a `critical` or `major` finding directly contradicts an `addressed` criterion (e.g., the criterion says "script is idempotent" but a major finding documents that the idempotency check is flawed), downgrade that criterion to `partiallyAddressed` and add a note explaining the gap. The acceptance criteria assessment and the findings must not send contradictory signals — an `addressed` criterion with a major finding against it is always a classification error. The spec analyst evaluates what the code does at face value; the reviewers evaluate whether it does it correctly. When they disagree, the reviewers' assessment takes precedence. If no `correlation` object is present, skip this step.
9. **Order by severity.** Within the findings array: all `critical` first, then `major`, then `minor`.
...
11. **Produce statistics.** Count total findings by severity after deduplication.
...
| file        | string or null   | File path — MUST be populated when the evidence snippet or description identifies the source file. Only use null when the finding genuinely cannot be attributed to a specific file. If the source reviewer provided a file, preserve it. If the evidence clearly comes from identifiable files (e.g., the code is from `useRedirectConfirmation.ts`), set the file field accordingly even if the source reviewer omitted it. When the evidence shows a function defined in a hook file but the finding says the parent component "implements" it, attribute the file to where the function is actually defined (the hook), not the component that imports it. |
