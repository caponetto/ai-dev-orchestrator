...
- **Permanent-state claims without lifecycle trace** — Before claiming an error state is permanent or never cleared, trace reconnect/`onopen`/cleanup handlers. If recovery clears the error, cap at `minor`.
- **Fallback imprecision** — "No fallback" must name which fallback is missing: build-time feature flag, runtime degradation to alternate data path, or reconnect with stale data preserved. Claiming "no fallback" when only one type is absent is overstated.
